        ...
        while Singleton.running:
            ...
            for key in workModes.keys():
                if Input.isKeyDown(key):
                    if currentWorkMode == workModes[key]:
                        continue
                    currentWorkMode = workModes[key]
                    if currentWorkMode.removePoints:
                        workPoints = []
                    selectedPoints = []

            for key in pointModes.keys():
                if Input.isKeyDown(key):
                    if currentPointMode == pointModes[key]:
                        continue
            ...
            match currentWorkMode.name:
                ...
                case "edit":
                    ...
                    if not Input.mouseDown:
                        if (
                            Vector.arrayDist(Input.mousePos, startMousePos)
                            < displaySize
                        ):
                ...
                case "delete":
                    point = PointMass(
                        None, PointInfo(Camera.inverseMap(Input.mousePos))
                    )
            ...
            def getClickedPoints():
                ...
                for point in workPoints:
                    if (
                        Vector.getMagnitude(Input.mousePosReal - point.position, True)
                        < displaySize**2
                    ):
            ...
            for i in range(-1, len(workPoints) - 1):
                ...
                pygame.draw.line(
                    Singleton.screen,
                    [0, 0, 0],
                    Camera.reMap(point.position),
                    Camera.reMap(nextpoint.position),
                    displaySize,
                )
            ...
            pygame.draw.circle(
                Singleton.screen, [0, 255, 0] if isSaved else [255, 0, 0], [15, 15], 7.5
            )

            if Input.isKeyDown("enter") and len(workPoints) >= 3:
                levelObjects.append(
                    GameObject([0, 0], [p.pointInfo for p in workPoints])
                )
                workPoints = []
                isSaved = False
    ...
    def drawGrid(spacing=100):
        ...
        for i in range(int(number[1]) + 2):
            ...
            pygame.draw.line(
                Singleton.screen,
                [100, 100, 100],
                Camera.reMap([topLeft[0], yCoord]),
                Camera.reMap([bottomRight[0], yCoord]),
                2,
            )
        ...
        for i in range(int(number[0]) + 2):
            ...
            pygame.draw.line(
                Singleton.screen,
                [100, 100, 100],
                Camera.reMap([xCoord, topLeft[1]]),
                Camera.reMap([xCoord, bottomRight[1]]),
                2,
            )
