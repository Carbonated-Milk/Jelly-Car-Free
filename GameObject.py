...
class GameObject:
    def __init__(self, position, pointInfo, frictionConst=0.5, offset=0):
        self.offset = offset
        ...
        for s in range(subdivide):
            for i in range(len(pointInfo) - 1, 0, -1):
                pointInfo.insert(
                    ...
                    PointInfo(
                        (
                            np.add(
                                np.array(pointInfo[i].position),
                                np.array(pointInfo[i - 1].position),
                            )
                            / 2
                        ).tolist()
                    ),
            ...
            pointInfo.insert(
                ...
                PointInfo(
                    (
                        np.add(
                            np.array(pointInfo[0].position),
                            np.array(pointInfo[-1].position),
                        )
                        / 2
                    ).tolist()
                ),
    ...
    def getCenter(self):
        ...
        if Settings.debugMode:
            pygame.draw.circle(
                Singleton.screen,
                [0, 255, 0],
                Camera.reMap(self.position.tolist(), self.offset),
                5,
            )
        ...
        for i in range(len(self.points)):
            ...
            if Settings.debugMode:
                pygame.draw.line(
                    surface,
                    [0, 255, 255],
                    Camera.reMap(self.points[i].getPosition(), self.offset),
                    Camera.reMap(
                        np.add(
                ...
                for connection in self.points[i].connections:
                    pygame.draw.line(
                        surface,
                        [255, 0, 0],
                        Camera.reMap(self.points[i].getPosition()),
                        Camera.reMap(connection.otherPoint.getPosition(), self.offset),
                    )
        if Settings.debugMode:
            pygame.draw.lines(
                surface, [0, 0, 0], True, Camera.reMap(points, self.offset), 5
            )
            pygame.draw.polygon(
                surface,
                [0, 255, 0],
            ...
            pygame.draw.polygon(
                surface, self.color, Camera.reMap(points, self.offset), 0
            )
            pygame.draw.lines(
                surface, [0, 0, 0], True, Camera.reMap(points, self.offset), thickness
            )
            for point in points:
                pygame.draw.circle(
                    surface, [0, 0, 0], Camera.reMap(point, self.offset), thickness / 2
                )
    ...
    def doCollisions(self, objects):
        for obj in objects:
            if obj.tag in self.noCollide:
                continue
            if obj is self:
                continue
            if not self.inBoundingBox(obj):
                continue
            self.collision(obj)
    ...
    def fixOverlap(self, point, lines):
        ...
        lerpPos = (point.getPosition()[0] - line.end1.getPosition()[0]) / (
            line.end2.getPosition()[0] - line.end1.getPosition()[0] + 0.001
        )
        ...
        virtualVelocity = (
            line.end1.velocity * line.end1.mass / line.getMass()
            + line.end2.velocity * line.end2.mass / line.getMass()
        )
        linepV = Vector.scalerProject(moveVec, virtualVelocity)
        pointpV = Vector.scalerProject(moveVec, point.velocity)
        newLineBouncyVelocity = (
            line.getMass() * linepV - point.mass * linepV + 2 * point.mass * pointpV
        ) / (line.getMass() + point.mass)
        newPointBouncyVelocity = (
            point.mass * pointpV
            - line.getMass() * pointpV
            + 2 * line.getMass() * linepV
        ) / (line.getMass() + point.mass)

        stiffVelocity = (line.getMass() * linepV + point.mass * pointpV) / (
            line.getMass() + point.mass
        )

        bounciness = 0.1
        pointTraction = 0.8
        newLineVelocity = newLineBouncyVelocity * bounciness + stiffVelocity * (
            1 - bounciness
        )
        newPointVelocity = newPointBouncyVelocity * bounciness + stiffVelocity * (
            1 - bounciness
        )

        lineVelocity = newLineVelocity * moveVec + (
            virtualVelocity - Vector.project(moveVec, virtualVelocity)
        ) * (1 - pointTraction) ** (
            abs(linepV - newLineVelocity) * np.linalg.norm(moveVec)
        )
        pVelocity = newPointVelocity * moveVec + (
            point.velocity - Vector.project(moveVec, point.velocity)
        ) * (1 - pointTraction) ** (
            abs(pointpV - newPointVelocity) * np.linalg.norm(moveVec)
        )

        point.velocity = pVelocity
        line.end1.velocity = (
            lineVelocity + (line.end1.velocity - line.end2.velocity) / 2
        )
        line.end2.velocity = (
            lineVelocity - (line.end1.velocity - line.end2.velocity) / 2
        )
        ...
        pygame.draw.line(
            ...
            [255, 0, 255],
            point.getPosition(),
            Vector.addArrays([-pVelocity, point.getPosition()]),
        )
    ...
    def calcBoundingBox(self):
        ...
        for point in self.points:
            ...
            if position[0] > topRight[0]:
                topRight[0] = position[0]
            elif position[0] < bottomLeft[0]:
                bottomLeft[0] = position[0]
            if position[1] > topRight[1]:
                topRight[1] = position[1]
            elif position[1] < bottomLeft[1]:
    ...
    def countInterSections(self, point, lines):
        numInterSects = 0
        for line in lines:
            if line.doesIntersectRight(point):
                numInterSects += 1
    ...
    def getClosestLine(self, point, lines):
        ...
        for line in lines:
            ...
            if (
                dist < closest
            ):  # lastOnLine and not onLine and newRealDist < closest or lastOnLine and onLine and dist < closest or not lastOnLine and onLine and dist < realDist or not lastOnLine and not onLine and newRealDist < realDist:# and newRealDist < realDist):
                closest = dist
                closestLine = line
                closestPos = closePos
                moveVec = vecDif
...
class Line:
    ...
    def doesIntersectRight(self, point, screen=None):
        ...
        if screen != None:
            pygame.draw.circle(
                screen, [255, 50, 255], Camera.reMap([xVal, checkPos[1]]), 5
            )
    ...
    def projectOnLine(self, vel):
        ...
        projectedDirection = (
            np.dot(vel, direction) / np.dot(direction, direction) * direction
        )
        if np.dot(vel, direction) < 0:
            projectedDirection = -projectedDirection
        return projectedDirection

    def projectPointOnLine(self, point, findClosest=False):
        direction = self.end1.getDirection(
            self.end2
        )  # vector from point 1 on line to point 2 on line
        relDirection = self.end1.getDirection(point)  # vector from point 1 to point
        projectedDirection = (
            np.dot(relDirection, direction) / np.dot(direction, direction) * direction
        )  # project vector on line
        projPos = np.add(
            projectedDirection, self.end1.getPosition()
        )  # add point 1 to get real world position

        if findClosest:
            dif1 = (
                self.end1.getPosition()[1] - projPos[1]
            )  # check that y's are the around the point
            dif2 = self.end2.getPosition()[1] - projPos[1]
            dif1y = (
                self.end1.getPosition()[0] - projPos[0]
            )  # check that y's are the around the point
            dif2y = self.end2.getPosition()[0] - projPos[0]
            if np.sign(dif1) == np.sign(dif2) and np.sign(dif1y) == np.sign(dif2y):
                projPos = (
                    self.end1.position
                    if self.end1.getDist(point) < self.end2.getDist(point)
                    else self.end2.position
                )

        return (
            projPos,
            np.linalg.norm(point.position - projPos),
            projPos - point.position,
        )
    ...
    def draw(self, color=[0, 0, 0], width=1):
        pygame.draw.line(
            Singleton.screen,
            color,
            self.end1.getPosition(),
            self.end2.getPosition(),
            width,
        )
